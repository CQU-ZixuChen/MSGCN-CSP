...
class Net(torch.nn.Module):
    def __init__(self, args):
        ...
        self.poolp1 = CSPool(self.nhid, ratio=self.pooling_ratio)
        self.pooln1 = CSPool(self.nhid, ratio=self.pooling_ratio)

        self.conv2 = ChebConv(self.nhid, self.nhid, 1)

        self.poolp2 = CSPool(self.nhid, ratio=self.pooling_ratio)
        self.pooln2 = CSPool(self.nhid, ratio=self.pooling_ratio)

        self.conv3 = ChebConv(self.nhid, self.nhid, 1)

        self.poolp3 = CSPool(self.nhid, ratio=self.pooling_ratio)
        self.pooln3 = CSPool(self.nhid, ratio=self.pooling_ratio)
    ...
    def forward(self, data):
        ...
        x1 = torch.cat([gmp(x_p1, batch_p1), gap(x_p1, batch_p1)], dim=1) # 1st GCN-Pooling features
        ...
        x2 = torch.cat([gmp(x_p2, batch_p2), gap(x_p2, batch_p2)], dim=1) # 2nd GCN-Pooling features
        ...
        x3 = torch.cat([gmp(x_p3, batch_p3), gap(x_p3, batch_p3)], dim=1) # 3rd GCN-Pooling features
        n3 = torch.cat([gmp(x_n3, batch_n3), gap(x_n3, batch_n3)], dim=1)

        x = x1 + x2 + x3
        # Total readout features
        x4 = x
        ...
        return x, n, score_n1, score_p1, score_n2, score_p2, score_n3, score_p3, x1, x2, x3, x4, x5
